...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const MaterialApp(home: CastSample());
  }
}
...
class _CastSampleState extends State<CastSample> {
  ...
  final _youtubeController = YoutubePlayerController.fromVideoId(
    videoId: YoutubePlayerController.convertUrlToId("https://youtu.be/DW5uloeg68o") ?? '',
    autoPlay: false,
  ...
  Future<void> _onPlayerStateChanged(dynamic state) async {
    // if the state is
    if (state == true) print(state.toString());
    if (state == true) {
      await _controller.loadMedia('https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4',
          title: "TestTitle", subtitle: "test Sub title", image: "https://smaller-pictures.appspot.com/images/dreamstime_xxl_65780868_small.jpg");
      await _controller.play();
    }
  }
...
class _RoundIconButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return IconButton(icon: Icon(icon, color: Colors.white), padding: const EdgeInsets.all(16.0), color: Colors.blue, onPressed: onPressed);
  }
}
